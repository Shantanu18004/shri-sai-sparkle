...
function LiveGoldRates() {
  ...
  const sorted = useMemo(() => {
    if (!rates) return [];
    const order = ["24K", "22K", "18K"];
    return order.map((p) => rates.find((r) => r.purity === p)).filter(Boolean);
  }, [rates]);
  ...
  return (
    <section className="border-b border-gold/20 bg-dark py-8 text-dark-foreground">
      <div className="mx-auto max-w-7xl px-6">
        <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
          <div className="flex items-center gap-3">
            <div className="grid h-10 w-10 place-items-center rounded-full border border-gold/40 bg-gold/10 text-gold">
              <TrendingUp className="h-5 w-5" />
            </div>
            <div>
              <div className="text-xs font-medium tracking-[0.2em] text-gold">LIVE GOLD RATES</div>
              <div className="text-[11px] text-dark-foreground/60">Per gram in Indian Rupees</div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            {sorted.map((r) => (
              <div
                key={r!.id}
                className="flex items-center gap-2 rounded border border-gold/30 bg-dark/60 px-4 py-2"
              >
                <span className="text-xs font-semibold tracking-wider text-gold">{r!.purity}</span>
                <span className="font-serif text-lg font-bold text-dark-foreground">
                  ₹{Number(r!.rate_per_gram).toLocaleString("en-IN")}
                </span>
                <span className="text-[10px] text-dark-foreground/50">/g</span>
              </div>
            ))}
            {sorted.length === 0 && (
              <span className="text-sm text-dark-foreground/50">Rates loading…</span>
            )}
          </div>

          <div className="flex items-center gap-2 text-[11px] text-dark-foreground/50">
            <RefreshCw className="h-3 w-3" />
            {latest ? (
              <span>Last updated {new Date(latest).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}</span>
            ) : (
